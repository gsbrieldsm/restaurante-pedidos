...
import {
  ...
  X, TrendingUp, Utensils, LayoutDashboard,
...
export default function PlanosPage() {
  ...
  const [planoSelecionado, setPlanoSelecionado] = useState<Tier>('pro')
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="grid sm:grid-cols-3 gap-4 mb-5">
          {PLANOS.map((p) => {
            ...
            return (
              <button
                key={p.id}
                onClick={() => setPlanoSelecionado(p.id as Tier)}
          ...
          {FEATURES.map((cat) => (
            <div key={cat.categoria}>
              ...
              {cat.itens.map((item) => (
                <div
                  key={item.texto}
                  className={`grid grid-cols-4 items-center px-6 py-2.5 border-b border-slate-50 ${
                    item.destaque ? 'bg-teal-50/60' : ''
                  }`}
                >
                  <div className="flex items-center gap-2 pr-4">
                    {item.destaque && (
                      <span className="shrink-0 text-xs font-bold text-teal-600 bg-teal-100 px-1.5 py-0.5 rounded">
                        destaque
                      </span>
                    )}
                    <span className={`text-sm ${item.destaque ? 'font-bold text-slate-800' : 'text-slate-600'}`}>
                      {item.texto}
                    </span>
                  </div>
        ...
        <div className="rounded-2xl px-6 py-4 flex items-center justify-between mb-5"
          ...
          <div>
            ...
            <p className="text-slate-400 text-xs mt-0.5">Redes, franquias e grandes operações — montamos um plano e escopo personalizados.</p>
          </div>
          <a
            href="https://wa.me/5547988194822?text=Ol%C3%A1%2C%20preciso%20de%20um%20plano%20enterprise%20personalizado%20para%20minha%20opera%C3%A7%C3%A3o"
